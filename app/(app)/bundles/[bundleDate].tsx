...
//   userRole,
//   requestStatus,
//   isRequestView, // New prop to indicate if we're in request view
// }: {
//   item: BundleType;
//   isSelected: boolean;
//   onSelect: () => void;
...
//       onPress={isExpired || isRequestView ? undefined : onSelect} // Disable onPress in request view
//       overflow="hidden"
//       accessible
...
//           {!isExpired && !requestStatus && !isRequestView && ( // Hide selection UI in request view
//             <View
...
//       if (id && requestData?.requests?.length > 0) {
//         // Filter bundles to only show the one associated with the request
//         const request = requestData.requests.find((req: any) => req._id === id);
//         if (request && request.bundleId?._id) {
//           const bundle = bundleData.bundles.find((b: BundleType) => b._id === request.bundleId._id);
//           setBundles(bundle ? [bundle] : []);
//         } else {
//           setBundles([]);
//         }
//       } else {
...
//     }
//   }, [refetchBundles, refetchRequests]);
...
//     ...bundle,
//     requestStatus: requestData?.requests?.find((req: any) => req.bundleId._id === bundle._id)?.status,
//   }));
...
//         isRequestView={!!id} // Pass isRequestView prop
//       />
//     );
//   };
...
//     !allExpired && 
//     !id; // Don't show accept button if viewing specific request

//   return (
//     <YStack flex={1} backgroundColor="$grey6">
//       <Header title={id ? "Requests" : "Available Bundles"} />

//       {(isBundleLoading || isRequestLoading) && !isRefreshing ? (
...
//               </Text>
//               <Text fontSize="$4" color="$gray10" textAlign="center" fontFamily="$body">
//                 Failed to load {id ? "requests" : "bundles"}. Please check your connection.
//               </Text>
//             </YStack>
//             <Button 
...
//               <YStack alignItems="center" gap="$2">
//                 <Text fontSize="$6" fontWeight="700" color="$black1" fontFamily="$heading">
//                   No {id ? "Requests" : "Bundles"} Available
//                 </Text>
...
const BundleScreen = () => {
  ...
  const isBundleView = type === "bundle";
  ...
  useEffect(() => {
    if (isBundleView && bundleData?.bundles) {
      ...
      // For request view, show only the bundle associated with the request
      const request = requestData.requests.find((req: any) => req._id === id);
      if (request && request.bundleId?._id) {
        // Assuming bundleId contains the full bundle object or we need to fetch it
        const bundle = request.bundleId; // Adjust based on actual API response structure
        setBundles(bundle ? [bundle] : []);
      } else {
        setBundles([]);
      }
